...
public class CreateResultAggregationStrategy implements AggregationStrategy {
  ...
  Logger logger = LoggerFactory.getLogger(CreateResultAggregationStrategy.class);
     ...
     * 
     * TODO: Figure out how to handle failed creations
    ...
    public Exchange aggregate(Exchange oldExchange, Exchange newExchange) {
    	...
    	logger.info("Aggregating Created Cic Parts. Req Guid: " + newExchange.getIn().getHeader("C3M_ReqGuid"));
    	logger.debug("Aggregating Created Cic Parts. Result received for Aggregation: " + newExchange.getIn().getBody(String.class));
        ...
        if (oldExchange == null) {
            ...
            try {
            	...
            	OnDemandOutboundCicRequest cicReq = mapper.readValue(aggregateExchange.getIn().getBody(String.class), OnDemandOutboundCicRequest.class);
 
                // Convert NewExchange Message to Java Object, and get the CicPart from it
            	OnDemandOutboundCicRequest cicReq2 = mapper.readValue(newExchange.getIn().getBody(String.class), OnDemandOutboundCicRequest.class);
