...
/**
 * This processor validates a response message that is received for aggregation after the deliver
 * step is complete. It ensures that no issues are experienced downstream
 * 
...
public class PreDeliverAggregationValidator implements Processor {
  ...
  public void process(Exchange exchange) throws Exception {
    ...
    logger.info("CCM Core :: Get the number of Deliver Specs in the deliver cic response message");
    ...
    if (count <= 0) {
      throw new RuntimeException(
          "Delivered CIC message cannot be aggregated. No Deliver Specs found");
    }
