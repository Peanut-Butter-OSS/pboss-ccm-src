...
/**
 * This class contains methods that allow us to split the CIC request message. It was built
 * specifically so the the split result contains the entire original message, but only one element
 * of a repeating section. For example: If a message comes in with many Delivery Specs, then we may
 * split it into multiple messages, each containing only a single delivery spec.
 * 
 * @author gregf
 *
 */
...
public class RequestSplitter {
  ...
  /**
   * Splits an OnDemandOutboundCicRequest into a List of requests, each containing exactly one
   * CicPart.
   * 
   * NOTE: This approach to splitting requires the creation of multiple clones of a message. I've
   * chosen to use Jackson to easily create clones instead of coding a clone method in the
   * OnDemandOutboundCicRequest bean.
   * 
   * @param body
   * @param headers
   * @return
  ...
  public List<String> splitByCicPart(String origBody) throws RuntimeException {
    ...
    try {
      ...
      OnDemandOutboundCicRequest cicReq =
          mapper.readValue(origBody, OnDemandOutboundCicRequest.class);
      ...
      if (count == 1) {
        ...
        for (int i = 0; i < count; i++) {
          ...
          OnDemandOutboundCicRequest cicReqClone =
              mapper.readValue(origBody, OnDemandOutboundCicRequest.class);
      ...
      String msg =
          "JsonParseException occurred while trying to split OnDemandOutboundCicRequest by CicPart. ";
      msg += "Exception message: " + e.getMessage();
      logger.error(msg, e);
      throw new RuntimeException(msg);
    } catch (JsonMappingException e) {
      String msg =
          "JsonMappingException occurred while trying to split OnDemandOutboundCicRequest by CicPart. ";
      msg += "Exception message: " + e.getMessage();
      logger.error(msg, e);
      throw new RuntimeException(msg);
    } catch (IOException e) {
      String msg =
          "IOException occurred while trying to split OnDemandOutboundCicRequest by CicPart. ";
      msg += "Exception message: " + e.getMessage();
      logger.error(msg, e);
      throw new RuntimeException(msg);
    }
    ...
    try {
      ...
      if (count == 0) {
        throw new RuntimeException(
            "Cannot split by Delivery Specs. No delivery specs found in the message");
      } else if (count == 1) {
        ...
        for (int i = 0; i < count; i++) {
          ...
          String deliverSpecGuid =
              cicReqClone.getCic().getDeliverSpecList().get(i).getDeliverSpecGuid();
          int left = cicReqClone.getCic().getDeliverSpecList().size();
          int n = 0;
          while (left > 1) {
            String compareGuid =
                cicReqClone.getCic().getDeliverSpecList().get(n).getDeliverSpecGuid();
      ...
      String msg =
          "JsonParseException occurred while trying to split OnDemandOutboundCicRequest by DeliverSpec. ";
      msg += "Exception message: " + e.getMessage();
      logger.error(msg, e);
      throw new RuntimeException(msg);
    } catch (JsonMappingException e) {
      String msg =
          "JsonMappingException occurred while trying to split OnDemandOutboundCicRequest by DeliverSpec. ";
      msg += "Exception message: " + e.getMessage();
      logger.error(msg, e);
      throw new RuntimeException(msg);
    } catch (IOException e) {
      String msg =
          "IOException occurred while trying to split OnDemandOutboundCicRequest by DeliverSpec. ";
      msg += "Exception message: " + e.getMessage();
      logger.error(msg, e);
      throw new RuntimeException(msg);
    } catch (RuntimeException e) {
      String msg =
          "RuntimeException occurred while trying to split OnDemandOutboundCicRequest by DeliverSpec. ";
      msg += "Exception message: " + e.getMessage();
      logger.error(msg, e);
      throw new RuntimeException(msg);
    }
